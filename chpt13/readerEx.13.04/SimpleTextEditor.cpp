...
//
// It also adds support for multi-line insertion with an 'A' command.
// Once in 'append' mode, multiple lines of text may be entered until the
// user enters a single period (.) on a line by itself.
//
// --------------------------------------------------------------------------
...
const std::string HEADER = "CS106B Programming Abstractions in C++: Ex 13.04\n";
const std::string DETAIL = "Simple Text Editor w A(ppend multi-line) command.";
    ...
    switch (toupper(line[0])) {
        case 'A': {
                ...
                while ((nextLine = getLine()) != END_OF_APPEND_MARKER) {
                    // Append one line at a time, adding back the newline
                    // that getLine normally strips off.
                    multiLine += nextLine + NEWLINE;
                }
                // Snip off one extra newline at end of non-null string.
...
void printHelpText() {
    ...
    cout << "  A     Append subsequent text until a " + END_OF_APPEND_MARKER;
