...
const std::string HEADER = "CS106B Programming Abstractions in C++: Ex 13.09\n";
const std::string DETAIL = "Stack-based editor with search feature. H for help.";
...
ParseResultT parseLine(const string & line, TokenScanner & scanner, int & repetition, ScaleT & scale, char & cmd, string & content);
void printHelpText();
...
void printHelpText() {
    ...
    cout << "  #|{F|B|D|C|X} Perform the command # times" << endl;
    cout << "  #|W{F|B|D|C|X} Perform the command # times on W boundaries" << endl;
