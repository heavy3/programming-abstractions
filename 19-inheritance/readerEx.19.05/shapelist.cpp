...
// elements.
//
// TODO: Get rid of build warning: 'Delete called on 'Shape' that is abstract
...
// window that contains the point (x, y).  If no matching shape if found,
// NULL is returned.
//
// More front-facing items are at the end of the containing vector,
// so we iterate from the end of the vector to the 0th element, looking
// for the first shape that contains the given coordinate.

Shape * ShapeList::getShapeAt(double x, double y) {
    Shape *sp = NULL;
    int maxi = this->size() - 1;
    for (int i = 0; i <= maxi; i++) {
        if ((*this)[maxi - i]->contains(x, y, PIXEL_PROX)) {
            sp = (*this)[maxi - i];
            break;
        }
    }
