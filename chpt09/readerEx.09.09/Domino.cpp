...
// Domino.cpp
//
// This file implements a domino class.
//
// --------------------------------------------------------------------------
// Attribution: "Programming Abstractions in C++" by Eric Roberts
// Chapter 6, Exercise 1.c
// Stanford University, Autumn Quarter 2012
// http://web.stanford.edu/class/archive/cs/cs106b/cs106b.1136/materials/CS106BX-Reader.pdf
// --------------------------------------------------------------------------
//
// Created by Glenn Streiff on 4/4/16
...
extern const int DFLT_DOMINO_RADIX = 7;

Domino::Domino(int theRadix) {
    leftDots = 0;
    rightDots = 0;
    radix = theRadix;
}

Domino::Domino(int lDots, int rDots, int theRadix) {
    leftDots = lDots;
    rightDots = rDots;
    radix = theRadix;
}
...
int Domino::getRank() {
    ...
    return msd * radix + lsd;
}

Domino Domino::flipped() {
    Domino d(radix);
    
    d.leftDots = getRightDots();
    d.rightDots = getLeftDots();
    
    return d;
}

bool Domino::chainsTo(Vector<Domino> dominos, int & index) {
    index = 0;
    ...
    if (this->getRightDots() == dominos[index].getLeftDots()) {
        return true;
    }
    
    index = dominos.size() - 1;
    if (dominos[index].getRightDots() == this->getLeftDots()) {
        index++;
        return true;
    }
