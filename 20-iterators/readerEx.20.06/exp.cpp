...
void FunctionExp::getVariables(Set<std::string>& varnames) {
    return fnArg->getVariables(varnames);
}
...
ExpressionFunction::~ExpressionFunction() {
    // Empty: Allow destructor intrinsic to the exp pointer to self
    //        destruct.
}
...
double ExpressionFunction::operator()(double varValue) {
    ...
    if (varnames.size() == 1) {
        ...
        error("ExpressionFunction: Expected variable missing in symbol table");
    }
